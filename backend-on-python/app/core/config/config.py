...
class Security(BaseModel):
    ...
    jwt_session_token_expire_secs: int = 24 * 3600  # 1d
    refresh_token_expire_secs: int = 28 * 24 * 3600  # 28d
    password_bcrypt_rounds: int = 12
    allowed_hosts: list[str] = ["localhost", "127.0.0.1"]
    backend_cors_origins: list[AnyHttpUrl] = []
    public_paths: list[str] = ["/docs", "/openapi.json", "/auth"]


class Database(BaseModel):
    hostname: str = "localhost"
    username: str = "jinaq"
    password: SecretStr = SecretStr("jinaq")
    port: int = 5432
    db: str = "default_db"
...
class LLM(BaseModel):
    ...
    max_universities_for_analysis: int = 10
    model: str = "gpt-3.5-turbo"
    max_tokens: int = 4096


class PG(BaseModel):
    email: str = "jinaq@jinaq.kz"
    password: str = "jinaq"
